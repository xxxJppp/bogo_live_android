...
public class VideoMaterialDownloadProgress {
    ...
    public void start(Downloadlistener listener) {
        ...
        HttpFileListener fileListener = new HttpFileListener() {
            ...
            public void onSaveSuccess(File file) {
                ...
                if (TextUtils.isEmpty(dataDir)) {
                    mListener.onDownloadFail(App.getApplication().getString(R.string.video_material_download_progress_material_unzip_failed));
                    stop();
                    return;
                }
                file.delete();
            ...
            public void onSaveFailed(File file, Exception e) {
                mListener.onDownloadFail(App.getApplication().getString(R.string.video_material_download_progress_download_failed));
                stop();
            }
        ...
        if (onlineMaterialDir == null || onlineMaterialDir.getName().startsWith("null")) {
            mListener.onDownloadFail(App.getApplication().getString(R.string.video_material_download_progress_no_enough_storage_space));
            stop();
            return;
        }
