 ...
 */
public class TCVideoEditerActivity extends BaseActivity implements
...
//        EventBus.getDefault().register(this);
        ...
        if (mTXVideoEditer == null || wrapper.getTXVideoInfo() == null) {
            Toast.makeText(this,
                    getResources().getString(R.string.tc_video_editer_activity_oncreate_status_is_abnormal_finish_editing),
...
//    @Subscribe
//    public void getEventBus(Integer msg) {
//        if (msg == TCConstants.EVENT_MSG_PUBLISH_DONE) {
//            clearConfig();
//            finish();
//        }
//    }
...
//            startActivity(intent);
            ...
            intent.putExtra(XRPublishVideoActivity.EXTRA_VIDEO_URL, mVideoOutputPath);
            startActivity(intent);
//        } else {
...
//                .setCancelable(false)
//                .addSheetItem(getResources().getString(R.string.tc_video_editer_activity_show_publish_dialog_save),
...
//                            }
//                        })
//                .addSheetItem(getResources().getString(R.string.tc_video_editer_activity_show_publish_dialog_publish),
//                        ActionSheetDialog.SheetItemColor.Blue, new ActionSheetDialog.OnSheetItemClickListener() {
//                            @Override
//                            public void onClick(int which) {
//                                mPublish = true;
        ...
        AlertDialog alertDialog = builder.setTitle(getString(R.string.tips)).setCancelable(false).setMessage(R.string.confirm_cancel_edit_content)
                .setPositiveButton(R.string.btn_back, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                        // 注意不能在这里调用clearConfig中的mTXVideoEditer.release()；放在onDestroy中
                        // 原因：先点击返回键则执行了clearConfig中的mTXVideoEditer.release释放资源，那么在之后的onPause生命周期中mTXVideoEditer.stopPlay将不执行
//                        clearConfig();
                        finish();
                    }
                })
    ...
    private void stopGenerate() {
        if (mCurrentState == PlayState.STATE_GENERATE) {
            mWorkLoadingProgress.dismiss();
            Toast.makeText(TCVideoEditerActivity.this,
                    getResources().getString(R.string.tc_video_editer_activity_cancel_video_generation),
                    Toast.LENGTH_SHORT).show();
            mWorkLoadingProgress.setProgress(0);
    ...
    private void addTailWaterMark() {
        ...
        Bitmap tailWaterMarkBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.icon);
        ...
        public void onCallStateChanged(int state, String incomingNumber) {
            ...
            switch (state) {
                ...
                case TelephonyManager.CALL_STATE_OFFHOOK:  //电话接听
                    ...
                    // 直接停止播放
                    Log.e(TAG, "onCallStateChanged stopPlay");
                    activity.stopPlay();
                    break;
                //电话挂机
                case TelephonyManager.CALL_STATE_IDLE:
                    // 重新开始播放
                    Log.e(TAG, "onCallStateChanged restartPlay");
