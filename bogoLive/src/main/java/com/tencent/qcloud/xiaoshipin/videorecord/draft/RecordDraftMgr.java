...
public class RecordDraftMgr {
    ...
    public RecordDraftInfo getLastDraftInfo(){
        String draftStr = mSharedPreferencesHelper.getSharedPreference(TCConstants.SP_KEY_RECORD_LAST_DRAFT, "").toString();
        if(TextUtils.isEmpty(draftStr)){
            return null;
        }
        Gson gson = new Gson();
        RecordDraftInfo recordDraftInfo = gson.fromJson(draftStr, new TypeToken<RecordDraftInfo>(){}.getType());
