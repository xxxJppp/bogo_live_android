...
import com.bogokj.live.adapter.LiveTabHotAdapter;
...
import com.bogokj.live.adapter.LiveTabNearbyAdapter;
...
public class LiveTabNearbyView extends LiveTabBaseView {
    ...
    private void init() {
        ...
        //    rv_content.addDividerHorizontal(new SDDrawable().size(SDViewUtil.dp2px(2)).color(SDResourcesUtil.getColor(R.color.red)));
        //    rv_content.addDividerVertical(new SDDrawable().size(SDViewUtil.dp2px(2)).color(SDResourcesUtil.getColor(R.color.red)));
        setAdapter();
            ...
            protected void onSuccess(final SDResponse resp) {
                if (actModel.isOk()) {
                    synchronized (LiveTabNearbyView.this) {
                        List<LiveRoomModel> dataList = actModel.getList();
                        sortData(dataList);

                        mListModel.clear();
                        mListModel.addAll(dataList);
                        mAdapter.notifyDataSetChanged();
                        mAdapter.setPageAndType(2, 1, 0, "");
                    }
                }
            }
