...
import com.bogokj.library.utils.SDThread;
...
import com.bogokj.live.music.lrc.LrcParser;
import com.bogokj.live.music.lrc.LrcRow;
 ...
 * 直播间播放音乐view基类
 */
public abstract class ARoomMusicView extends RoomView implements LivePlayMusicView.ClickListener, PushMusicEffectDialog.MusicEffectCallback
{
    private LivePlayMusicView view_play_music;
    private SDThread mLrcThread;
    private MusicEffectConfig mMusicEffectConfig;
    ...
    public MusicEffectConfig getMusicEffectConfig()
    {
        return mMusicEffectConfig;
    }

    private void initPlayCtrl()
    {
        mMusicEffectConfig = SDDisk.open().getSerializable(MusicEffectConfig.class);
        if (mMusicEffectConfig == null)
        {
            mMusicEffectConfig = new MusicEffectConfig();
            SDDisk.open().putSerializable(mMusicEffectConfig);
        }
    }

    @Override
    public void onMusicEffect(MusicEffectConfig config)
    {
        mMusicEffectConfig = config;
        updateEffectValue(config);
    }

    /**
     * 更新音效参数，音量
     */
    protected abstract void updateEffectValue(MusicEffectConfig config);
