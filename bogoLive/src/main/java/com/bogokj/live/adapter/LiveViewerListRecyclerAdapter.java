...
public class LiveViewerListRecyclerAdapter extends SDSimpleRecyclerAdapter<UserModel> {
    ...
    public LiveViewerListRecyclerAdapter(Activity activity, ILiveInterface liveActivity) {
        super(activity);
    }
    ...
    public void onBindData(SDRecyclerViewHolder<UserModel> holder, int position, final UserModel model) {
        ...
        ImageView iv_guardian = holder.get(R.id.iv_guardian);

        // 守护者标志
        if (model.getIs_guardian() == 1) {
            SDViewUtil.setVisible(iv_guardian);
        } else {
            SDViewUtil.setGone(iv_guardian);
        }
