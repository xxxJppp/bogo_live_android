...
public class LiveSetBeautyFilterAdapter extends SDSimpleAdapter<LiveBeautyFilterModel>
{
    ...
    public void bindData(final int position, View convertView, ViewGroup parent, final LiveBeautyFilterModel model)
    {
        ...
        if (model.isSelected())
        {
            ...
            SDViewUtil.setTextViewColorResId(tv_name, R.color.res_main_color);
        } else
        {
            SDViewUtil.setBackgroundResource(tv_name, R.drawable.layer_bg_beauty_filter_normal);
            SDViewUtil.setTextViewColorResId(tv_name, R.color.res_text_gray_s);
        }
