...
import com.bogokj.live.adapter.LiveRechrgeDiamondsPaymentRuleAdapter;
...
public class LiveRechargeDiamondsActivity extends BaseTitleActivity {
    ...
    private List<PayItemModel> listPayWay = new ArrayList<>();

    //payRule
    private LiveRechrgeDiamondsPaymentRuleAdapter mAdapterPaymentRule;
    private List<RuleItemModel> mListCommonPaymentRule = new ArrayList<>();

    private TextView tv_diamonds_name;

    private ImageView im_charge_diamond;
    ...
    private boolean isSelectedRule = false;
    ...
    protected void init(Bundle savedInstanceState) {
        ...
        tv_diamonds_name = findViewById(R.id.tv_diamonds_name);
        im_charge_diamond = findViewById(R.id.im_charge_diamond);
        im_charge_diamond.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void initPaymentRule() {
        ...
        mAdapterPaymentRule.setItemClickCallback(new SDItemClickCallback<RuleItemModel>() {
            ...
            public void onItemClick(int position, RuleItemModel item, View view) {
                ...
                for (RuleItemModel model : mListCommonPaymentRule) {
        ...
        lv_payment_rule.setSpanCount(3);
    ...
    private void requestData() {
        CommonInterface.requestRecharge(new AppRequestCallback<App_rechargeActModel>() {
            ...
            protected void onSuccess(SDResponse resp) {
                if (actModel.isOk()) {
                    if (actModel.getRule_list() != null && actModel.getRule_list().size() > 0) {
                        mListCommonPaymentRule.clear();
                        mListCommonPaymentRule.addAll(actModel.getRule_list());
                    }

                    if (actModel.getPay_list() != null && actModel.getPay_list().size() > 0) {
                        listPayWay.clear();
                        listPayWay.addAll(actModel.getPay_list());
                    }
