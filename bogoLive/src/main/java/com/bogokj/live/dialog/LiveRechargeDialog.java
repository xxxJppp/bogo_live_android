...
import com.bogokj.live.appview.LiveGameExchangeView;
import com.bogokj.live.appview.LiveRechargeDiamondsView;
...
import com.bogokj.live.model.App_rechargeActModel;
...
//    private boolean openExchange = true;
    ...
    private void initTabs() {
        if (!AppRuntimeWorker.isUseGameCurrency()) {
            SDViewUtil.setGone(ll_tab_game_exchange);
            SDViewUtil.setGone(view_stroke_game_exchange);
        }
        tab_recharge.setTextTitle("充值");
        tab_recharge.getViewConfig(tab_recharge.mIvUnderline).setBackgroundColorNormal(Color.TRANSPARENT).setBackgroundColorSelected(SDResourcesUtil.getColor(R.color.res_main_color));
        ...
        tab_recharge.getViewConfig(tab_recharge.mTvTitle).setTextColorNormal(SDResourcesUtil.getColor(R.color.res_text_gray_s)).setTextColorSelected(SDResourcesUtil.getColor(R.color.res_main_color));

        tab_game_exchange.setTextTitle("兑换");
        tab_game_exchange.getViewConfig(tab_game_exchange.mIvUnderline).setBackgroundColorNormal(Color.TRANSPARENT).setBackgroundColorSelected(SDResourcesUtil.getColor(R.color.res_main_color));
        ...
        tab_game_exchange.getViewConfig(tab_game_exchange.mTvTitle).setTextColorNormal(SDResourcesUtil.getColor(R.color.res_text_gray_s)).setTextColorSelected(SDResourcesUtil.getColor(R.color.res_main_color));
    ...
    private void closeDialog() {
        ...
        if (mViewExchange != null && view == mViewExchange) {
            ...
            if (mRechargeHandCloseListener != null) {
                mRechargeHandCloseListener.clickHandClose();
            }
            dismiss();
        }
    }
    ...
    private void requestPay(double money, int rule_id, int pay_id) {
        CommonInterface.requestPay(pay_id, rule_id, money, new AppRequestCallback<App_payActModel>() {
            ...
            protected void onStart() {
                super.onStart();
                AppDialogProgress.showWaitTextDialog(getContext(),"正在启动插件");
            }
            ...
            protected void onSuccess(SDResponse resp) {
                if (actModel.getStatus() == 1) {
                    ...
                    CommonOpenSDK.dealPayRequestSuccess(actModel, getOwnerActivity(), payResultListner);
                }
            }
        });
    }
...
//    };
    ...
    public void setmRechargeHandCloseListener(RechargeHandCloseListener mRechargeHandCloseListener) {
        this.mRechargeHandCloseListener = mRechargeHandCloseListener;
    }
