package com.bogokj.auction.model.custommsg;

import com.bogokj.auction.model.PaiBuyerModel;
import com.bogokj.live.LiveConstant;
import com.bogokj.live.model.UserModel;
import com.bogokj.live.model.custommsg.CustomMsg;

import java.util.List;

/**
 * Created by Administrator on 2016/8/16.
 */
public class CustomMsgAuctionSuccess extends CustomMsg
{
    private int room_id; //房间ID
    private int pai_id; //拍卖的项目ID
    private List<PaiBuyerModel> buyer;
    private int post_id;// 主播ID
    private String desc;//消息

    public CustomMsgAuctionSuccess()
    {
        super();
        setType(LiveConstant.CustomMsgType.MSG_AUCTION_SUCCESS);
    }

    public int getRoom_id()
    {
        return room_id;
    }

    public void setRoom_id(int room_id)
    {
        this.room_id = room_id;
    }

    public int getPai_id()
    {
        return pai_id;
    }

    public void setPai_id(int pai_id)
    {
        this.pai_id = pai_id;
    }

    public List<PaiBuyerModel> getBuyer()
    {
        return buyer;
    }

    public void setBuyer(List<PaiBuyerModel> buyer)
    {
        this.buyer = buyer;
    }

    public int getPost_id()
    {
        return post_id;
    }

    public void setPost_id(int post_id)
    {
        this.post_id = post_id;
    }

    public String getDesc() {
        return desc;
    }

    public void setDesc(String desc) {
        this.desc = desc;
    }
}
