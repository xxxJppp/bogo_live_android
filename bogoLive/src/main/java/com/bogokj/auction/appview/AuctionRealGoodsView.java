...
import com.bogokj.auction.event.ECreateAuctionSuccess;
...
public class AuctionRealGoodsView extends AuctionGoodsBaseView implements OptionsPickerView.OnOptionsSelectListener
{
    ...
    private EditText et_auction_duration;//竞拍时间
    private LinearLayout ll_delay_time;
    private EditText et_delay_time;//延迟时间
    private LinearLayout ll_delay_times;
    private EditText et_delay_times;//最大延迟次数
...
//        ll_delay_times.setOnClickListener(this);
            ...
            protected void onSuccess(SDResponse sdResponse)
            {
                if (actModel.getStatus() == 1)
                {
                    if (actModel.getData() != null)
                    {
                        ...
                        SDViewBinder.setTextView(et_auction_duration, pai_time + "");
                        SDViewBinder.setTextView(et_delay_time, pai_yanshi + "");
                        SDViewBinder.setTextView(et_delay_times, max_yanshi + "");
                    }
                }
            }
    ...
    private void addParams()
    {
        ...
        mapParams.put(KEY_PAI_TIME, Double.valueOf(et_auction_duration.getText().toString().trim()));
        mapParams.put(KEY_PAI_YANSHI, Integer.valueOf(et_delay_time.getText().toString().trim()));
        mapParams.put(KEY_MAX_YANSHI, Integer.valueOf(et_delay_times.getText().toString().trim()));
