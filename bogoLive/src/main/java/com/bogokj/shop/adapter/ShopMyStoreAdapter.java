package com.bogokj.shop.adapter;
...
public class ShopMyStoreAdapter extends SDSimpleAdapter<App_shop_mystoreItemModel>
{
    ...
    public void bindData(final int position, View convertView, ViewGroup parent, final App_shop_mystoreItemModel model)
    {
        ...
        TextView tv_shop_push = ViewHolder.get(R.id.tv_shop_push, convertView);//推送
        View view_stroke_line = ViewHolder.get(R.id.view_stroke_line, convertView);

//        if (position == getCount() - 1)
//        {
//            SDViewUtil.setGone(view_stroke_line);
//        }

        if (!mIsCreater)
        {
            SDViewUtil.setGone(tv_shop_push);
        } else
        {
            SDViewUtil.setVisible(tv_shop_push);
        }
         ...
         * 推送
         */
        tv_shop_push.setOnClickListener(new View.OnClickListener()
        {
            @Override
