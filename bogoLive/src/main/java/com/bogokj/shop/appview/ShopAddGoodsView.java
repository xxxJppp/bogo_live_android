...
import com.bogokj.auction.appview.AuctionGoodsBaseView;
...
import com.bogokj.shop.model.App_shop_goodsActModel;
...
public class ShopAddGoodsView extends AuctionGoodsBaseView implements TextView.OnEditorActionListener, TextWatcher
{
    ...
    public void onEventMainThread(EUpLoadImageComplete event)
    {
        ...
        {
            mapPic.add(event.server_full_path);
    ...
    protected boolean isParamsComplete()
    {
        ...
        if (getEtContent(et_goods_price).length()>7){
            SDToast.showToast("你输入的金额超过当前限制");
            return false;
        }
        ...
        if(getEtContent(et_goods_des).length()>25){
            SDToast.showToast("商品描述不能超过25字");
            return false;
        }
        return true;
    }
